...
def main():
    ...
    argparser.add_argument(
        "--primary-ckpt",
        type=Path,
        default=Path(f"./checkpoints/BMR_ZSR_256_YZ/checkpoint.pth"),
        help="(optional) Path to primary model checkpoint.",
    )
    argparser.add_argument(
        "--auxiliary-ckpt",
        type=Path,
        default=Path(f"./checkpoints/BMR_ZSR_256_XY/checkpoint.pth"),
    ...
    argparser.add_argument(
        "--batch-size", type=int, default=6, help="(optional) Batch size for sampling."
    )
    argparser.add_argument(
        "--zsr-factor",
        type=int,
    ...
    argparser.add_argument(
        "--dps-weight", type=float, default=None, help="(optional) Weight of DPS step."
    )
    argparser.add_argument(
        "--K",
        type=tutils.int_or_float,
        default=2,
    ...
    if args.dps_weight is None:
        if args.zsr_factor in BMR_ZSR_DPS_WEIGHT_MAP:
            ...
            logging.warning(
                "Using auto DPS weight for the given ZSR factor. For the best performance, please specify the DPS weight manually."
            )
        else:
            raise ValueError(
                "There is no default DPS weight for the given ZSR factor. Please specify the DPS weight manually."
            )

    save_root = Path(
        f"./invp_results/{args.problem_name}/x{args.zsr_factor}/K{args.K}/lamb{args.dps_weight}/{datetime.datetime.now().strftime('%y%m%d_%H%M%S')}/"
    )
    ...
    score_model_pri, score_model_aux = tutils.get_tpdm_models(
        config, args.primary_ckpt, args.auxiliary_ckpt
    )
    ...
    tpdm_zsr = controllable_generation.get_tpdm_zsr(
        sde,
        predictor,
        corrector,
        inverse_scaler,
    ...
    for i, (fname, _) in enumerate(fname_list):
        ...
        plt.imsave(
            save_root / "measure" / f"{just_name}.png",
            clear(inverse_scaler(measure[i])),
            cmap="gray",
        )
        plt.imsave(
            save_root / "measure_dagger" / f"{just_name}.png",
            clear(inverse_scaler(measure_dagger[i])),
            cmap="gray",
        )
        plt.imsave(
            save_root / "label" / f"{just_name}.png",
            clear(inverse_scaler(label[i])),
            cmap="gray",
        )
    ...
    for recon_img, label_one, (fname, _) in zip(recon, label, fname_list):
        just_name = fname.split(".")[0]
        plt.imsave(
            save_root / "recon" / f"{just_name}.png", clear(recon_img), cmap="gray"
        )
        np.save(
            str(save_root / "label" / f"{just_name}.npy"),
            label_one.detach().cpu().numpy(),
        )
        np.save(
            str(save_root / "recon" / f"{just_name}.npy"),
