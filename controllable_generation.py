...
def get_tpdm_zsr(
    ...
    def decouple(inputs):
        ...
        inputs_rs = inputs[..., 0 : 256 - w_pad_size].reshape(
            B, C, H, W // factor, factor
        )
    ...
    def couple(inputs):
        ...
        inputs_cp_rs = torch.einsum(
            "bchwi,ij->bchwj", inputs_rs, invM.to(inputs.device)
        )
    ...
    def tpdm_zsr(model_pri, model_aux, measure_dagger):
        batch_len = (
            len(measure_dagger) // config.eval.batch_size
            if len(measure_dagger) % config.eval.batch_size == 0
            else len(measure_dagger) // config.eval.batch_size + 1
        )
        ...
        # Initial sample
        x = couple(
            decouple(measure_dagger) * mask
            + decouple(
                sde.prior_sampling(shape).to(measure_dagger.device) * (1.0 - mask)
            )
        )
        ...
        for i in tqdm(range(sde.N), colour="blue", unit="step", smoothing=0):
            ...
            for x_batch, gray_scale_img_batch in tqdm(
                zip(dataloader, dataloader_md),
                total=batch_len,
                colour="blue",
                unit="mb",
                leave=False,
                ...
                if primary_tern:  # reverse diffusion with primary model + DPS
                    t = timesteps[i]
                    x_batch, x_mean_batch = corrector_tpdm_zsr_update_fn(
                        model_pri, gray_scale_img_batch, x_batch, t
                    )
                    x_batch, x_mean_batch = predictor_tpdm_zsr_update_fn(
                        model_pri, gray_scale_img_batch, x_batch, t
                    )

                else:  # reverse diffusion with auxiliary model
                    t = timesteps[i]
                    x_batch, x_mean_batch = corrector_tpdm_uncond_update_fn(
                        model_aux, x_batch, t
                    )
                    x_batch, x_mean_batch = predictor_tpdm_uncond_update_fn(
                        model_aux, x_batch, t
                    )
            ...
            if save_progress:
                if (i % 50) == 0:
                    plt.imsave(
                        ...
                        clear(x_mean[len(x_mean) // 2 : len(x_mean) // 2 + 1]),
                        cmap="gray",
                    )
...
def get_tpdm_cs_mri(
    ...
    def tpdm_cs_mri(model_pri, model_aux, measure_kspace, mask):
        batch_len = (
            len(measure_kspace) // config.eval.batch_size
            if len(measure_kspace) % config.eval.batch_size == 0
            else len(measure_kspace) // config.eval.batch_size + 1
        )
        ...
        for i in tqdm(range(sde.N), colour="blue", unit="step", smoothing=0):
            ...
            for x_batch, measure_kspace_batch, mask_batch in tqdm(
                zip(dataloader, dataloader_measure_kspace, dataloader_mask),
                total=batch_len,
                colour="blue",
                unit="mb",
                leave=False,
                ...
                if primary_tern:  # reverse diffusion with primary model + DPS
                    t = timesteps[i]
                    x_batch, x_mean_batch = corrector_tpdm_cs_mri_update_fn(
                        model_pri, measure_kspace_batch, mask_batch, x_batch, t
                    )
                    x_batch, x_mean_batch = predictor_tpdm_cs_mri_update_fn(
                        model_pri, measure_kspace_batch, mask_batch, x_batch, t
                    )
...
def get_tpdm_sv_ct(
    ...
    def tpdm_sv_ct(model_pri, model_aux, measure_sino):
        batch_len = (
            len(measure_sino) // config.eval.batch_size
            if len(measure_sino) % config.eval.batch_size == 0
            else len(measure_sino) // config.eval.batch_size + 1
        )
        ...
        for i in tqdm(range(sde.N), colour="blue", unit="step", smoothing=0):
            ...
            for x_batch, measure_sino_batch in tqdm(
                zip(dataloader, dataloader_measure_sino),
                total=batch_len,
                colour="blue",
                unit="mb",
                leave=False,
                ...
                if primary_tern:  # reverse diffusion with primary model + DPS
                    t = timesteps[i]
                    x_batch, x_mean_batch = corrector_tpdm_sv_ct_update_fn(
                        model_pri, measure_sino_batch, x_batch, t
                    )
                    x_batch, x_mean_batch = predictor_tpdm_sv_ct_update_fn(
                        model_pri, measure_sino_batch, x_batch, t
                    )
...
def get_tpdm_uncond(
    ...
    def tpdm_uncond(model, model_aux):
        ...
        batch_len = (
            ...
            if B % config.eval.batch_size == 0
            else B // config.eval.batch_size + 1
        )
        ...
        for i in tqdm(range(sde.N), colour="blue", unit="step", smoothing=0):
            ...
            for x_batch in tqdm(
                dataloader, total=batch_len, colour="blue", unit="mb", leave=False
            ):
            ...
            if save_progress:
                if (i % 50) == 0:
                    plt.imsave(
                        save_root / "progress" / f"progress{i}.png",
                        clear(x_mean[len(x_mean) // 2 : len(x_mean) // 2 + 1]),
                        cmap="gray",
                    )
